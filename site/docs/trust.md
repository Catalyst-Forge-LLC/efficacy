...
Agents should act on tier 3 and 4 `use` records. If a check was not run, the tier stays lower and the report says what was unchecked.
...
2. Use only `use` lines at tier 3 or 4 that are not retracted. The `summary` line lists them.
